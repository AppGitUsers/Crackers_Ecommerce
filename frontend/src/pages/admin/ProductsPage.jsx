  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-extrabold text-brand-800">Products</h1>
        ...
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-sandal-100">
              {products.map((p) => (
                <tr key={p.id}>
                  <td className="px-4 py-3 font-semibold text-brand-900 flex items-center gap-2">
                  ...
                  <td className="px-4 py-3">
                    <button
                      className={`badge ${p.is_available ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    <button className="text-brand-600 font-semibold" onClick={() => openEdit(p.id)}>Edit</button>
                    <button className="text-red-500 font-semibold" onClick={() => handleDelete(p.id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-30 px-4 py-8 overflow-y-auto">
          <form onSubmit={handleSubmit} className="card p-6 w-full max-w-lg space-y-3">
            <h2 className="font-bold text-lg text-brand-800">{editing ? 'Edit Product' : 'New Product'}</h2>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="text-sm font-semibold text-brand-700">Price (₹)</label>
                <input type="number" step="0.01" className="input mt-1" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} required />
              </div>
              <div>
                <label className="text-sm font-semibold text-brand-700">Stock Qty</label>
              ...
              <div>
                <label className="text-sm font-semibold text-brand-700">Unit</label>
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="text-sm font-semibold text-brand-700">Available for sale</label>
            </div>

            {editing && (
              <div className="border-t border-sandal-200 pt-3">
                <label className="text-sm font-semibold text-brand-700">Photos</label>
                ...
                <p className="text-xs text-brand-400 mt-1">Take a photo or choose from gallery.</p>
              </div>
            )}
            {!editing && (
              <p className="text-xs text-brand-400 border-t border-sandal-200 pt-3">Save the product first, then add photos.</p>
            )}
